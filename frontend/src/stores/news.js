...
export const useNewsStore = defineStore('news', {
    ...
    actions: {
        ...
        async promptSearchNews(prompt) {
            ...
            try {
                const response = await axios.post(
                    'http://localhost:8000/api/v1/news/search_news',
                    { prompt: prompt }
                );
                this.newsList = response.data.map((news) => ({
                    ...news,
        ...
        async fetchNewsSummary(content, index) {
            ...
            try {
                const response = await axios.post(
                    'http://localhost:8000/api/v1/news/news_summary',
                    { content: content }
                );
                if (
                    response.data &&
                    index >= 0 &&
                    index < this.newsList.length
                ) {
        ...
        async toggleUpvote(newsId) {
            ...
            try {
                await axios.post(
                    `http://localhost:8000/api/v1/news/${newsId}/upvote`
                );
            } catch (error) {
